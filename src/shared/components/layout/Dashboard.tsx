...
import { 
  ChevronDown, Cpu, Wrench, AlertTriangle, ClipboardList, BarChart2, 
  Database, Network, GitCommit, Box, Activity, Bell
} from 'lucide-react';
import { 
...
export const Dashboard = ({
  ...
  const branchMultiplier = BRANCHES.indexOf(selectedBranch) + 1 || 1;

  const dynamicIncidentList = React.useMemo(() => {
    return [1, 2, 3, 4, 5].map((i) => {
      const type = ['Thiết bị', 'Đường dây', 'Trạm', 'Nút', 'Thiết bị'][i % 5];
      const deviceNames = ['MBA T1', 'ĐD 110kV', 'Cột 25', 'Ngăn lộ 171', 'Tủ RMU'];
      return {
        id: i,
        type,
        title: `Sự cố ${deviceNames[i % deviceNames.length]} - ${selectedBranch.split(' ').pop()}`,
        time: `${10 + (i * branchMultiplier) % 10}:30 ${20 + (i % 8)}/03/2026`,
        desc: [
          `Quá nhiệt tại thiết bị, cần kiểm tra ngay hệ thống làm mát.`,
          `Rạng nứt cách điện tại khu vực đường dây.`,
          `Sụt áp bất thường tại trạm biến áp.`,
          `Hỏng rơ le bảo vệ ngăn lộ, không thể đóng cắt tự động.`,
          `Rò rỉ khí SF6 tại tủ RMU, áp suất giảm dưới mức cho phép.`
        ][i % 5]
      };
    });
  }, [selectedBranch, branchMultiplier]);

  const dynamicCbmList = React.useMemo(() => {
    return [1, 2, 3, 4, 5].map((i) => ({
      id: i,
      title: `Chỉ đạo #${1000 + i * branchMultiplier}`,
      status: ['Đang xử lý', 'Chờ duyệt', 'Mới tạo', 'Đã hoàn thành', 'Mới tạo'][i % 5],
      date: `${10 + i}/03/2026`,
      desc: [
        `Thay thế sứ cách điện tại vị trí cột 15-16.`,
        `Bảo dưỡng định kỳ máy biến áp: lọc dầu, kiểm tra quạt.`,
        `Đo lại hệ thống tiếp địa tại khu vực trạm biến áp.`,
        `Vệ sinh công nghiệp thiết bị, kiểm tra xiết chặt các đầu cốt.`,
        `Kiểm tra phát nhiệt bằng camera nhiệt tại các điểm đấu nối.`
      ][i % 5]
    }));
  }, [selectedBranch, branchMultiplier]);

  const currentChartDataEqCurrent = MOCK_CHART_DATA_EQ_CURRENT.map(item => ({ ...item, value: item.value * branchMultiplier }));
  const currentChartDataEqLastMonth = MOCK_CHART_DATA_EQ_LAST_MONTH.map(item => ({ ...item, value: item.value * branchMultiplier }));
  const currentChartDataEqLastYear = MOCK_CHART_DATA_EQ_LAST_YEAR.map(item => ({ ...item, value: item.value * branchMultiplier }));
  
  const currentChartDataIncident3Months = MOCK_CHART_DATA_INCIDENT_3_MONTHS.map(item => ({
    ...item,
    thietBi: Math.floor(item.thietBi * (0.8 + branchMultiplier * 0.2)),
    duongDay: Math.floor(item.duongDay * (0.8 + branchMultiplier * 0.2)),
    tram: Math.floor(item.tram * (0.8 + branchMultiplier * 0.2)),
  }));

  const currentChartData3 = MOCK_CHART_DATA_3.map(item => ({
    ...item,
    anToan: Math.floor(item.anToan * (0.8 + branchMultiplier * 0.2)),
    vanHanh: Math.floor(item.vanHanh * (0.8 + branchMultiplier * 0.2)),
    suaChua: Math.floor(item.suaChua * (0.8 + branchMultiplier * 0.2)),
    thiNghiem: Math.floor(item.thiNghiem * (0.8 + branchMultiplier * 0.2)),
  }));
